...
- **按需加载**：所有弹窗组件（设置、编辑、确认、导入等）改为异步加载，首次打开时按需下载。
- **拼音懒加载**：`pinyin-pro` 库改为动态 import，仅在搜索或账号变更时加载，不占用启动时间。
- **移除预计算**：拼音缓存不再在启动时预构建，改为账号变化时按需计算，减少启动 CPU 开销。
- **分包优化**：`pinyin-pro` 独立拆分为异步 chunk，不混入首屏 vendor 包。
- **删除占位文字**：移除 `<div id="app">` 中的"正在启动插件"加载提示。
